function scoreMatch(text, queryTerms) {
...
export function searchGlossary(content, query) {
    ...
    const lines = top.map((item, i) => `${i + 1}. **${item.term.term}**\n${item.term.definition}`);
    return `Found ${top.length} glossary term(s) for "${query}":\n\n${lines.join("\n\n")}`;
}
export function getBlogPost(content, slug) {
    // Direct slug lookup
...
function formatBlogPost(post) {
    ...
    if (post.url)
        result += `**URL:** ${post.url}\n`;
    const metaKeys = Object.keys(post.metadata).filter((k) => !["Author", "Date", "URL"].includes(k));
...
export function getComparison(content, slug) {
    // Direct lookup
